...
def map_2_word_to_word(corpus):
    """Load list and use dictionary to map word-pair (2 words) to trailing word."""
    limit = len(corpus) - 2
    ...
    logging.debug("map_2_word_to_word results for \"sake jug\" = %s\n",
                  dict2_to_1['sake jug'])
    return dict2_to_1


def random_word(corpus):
    """Return a random word and syllable count from training corpus."""
    word = random.choice(corpus)
    num_syls = count_syllables(word)
    if num_syls > 4:
        random_word(corpus)
    else:
        logging.debug("random word and syllables = %s\n", word, num_syls)
...
def main():
    ...
    suffix_map_2 = map_2_word_to_word(corpus)
    final = []
    ...
    while choice != "0":
        print(
            ...
            0 - Quit
            1 - Generate fresh Haiku
            2 - Re-generate Line 2
            3 - Re-generate Line 3
            """
        )
